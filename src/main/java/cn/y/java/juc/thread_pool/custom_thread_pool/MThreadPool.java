package cn.y.java.juc.thread_pool.custom_thread_pool;
...
public class MThreadPool {
    ...
    public static class MBlockingQueue {
        ...
        public Runnable take() {
            ...
            try {
                while (queue.isEmpty()) {
                    try {emptyWaitSet.await();} catch (InterruptedException e) {throw new RuntimeException(e);}
                }
                Runnable task = queue.removeFirst();
                fullWaitSet.signal();
                return task;
